package com.example.thedeliverer;

import androidx.appcompat.app.AppCompatActivity;

import android.content.Intent;
import android.os.Bundle;
import android.view.View;
import android.widget.Button;
import android.widget.EditText;
import android.widget.Toast;

import com.example.thedeliverer.Database.DBHelperOrder;

import java.util.ArrayList;
import java.util.Collections;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

public class FoodCart extends AppCompatActivity {
    //Variables
    EditText item1,item2,item3,item4;
    EditText quantity1, quantity2,quantity3,quantity4;
    EditText size1,size2,size3,size4;
    String item_name1,item_name2,item_name3,item_name4,item_quantity1,item_quantity2,item_quantity3,item_quantity4,item_size1,item_size2,item_size3,item_size4;
    List<String> items;
    String ID1,ID2,ID3,ID4;
    int size;
    String cart_size;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_food_cart);
        //Referencing to resources in food cart layout with correspoding ID
        //Items
        item1 = findViewById(R.id.item1);
        item2 = findViewById(R.id.item2);
        item3 = findViewById(R.id.item3);
        item4 = findViewById(R.id.item4);
        //quantities
        quantity1 = findViewById(R.id.quantity1);
        quantity2 = findViewById(R.id.quantity2);
        quantity3 = findViewById(R.id.quantity3);
        quantity4 = findViewById(R.id.quantity4);
        //sizes
        size1 = findViewById(R.id.size1);
        size2 = findViewById(R.id.size2);
        size3 = findViewById(R.id.size3);
        size4 = findViewById(R.id.size4);
        //Buttons
        Button continue_btn = findViewById(R.id.btn_continue);
//        //Initializing maps
//        items_and_quantities = new HashMap<String, String>();
//        items_and_sizes = new HashMap<String, String>();

        ID1 = ID2 =ID3 = ID4 = null;

        //Initializing items arraylist
        items = new ArrayList<String>();
//        itemNames = new ArrayList<String>();

        //Retrieving intent from the dashboard
        Intent intent = getIntent();
        //Retrieving the food items from the dashboard
        items = (ArrayList<String>) intent.getSerializableExtra("food_items");

        Collections.sort(items);

        //size of the items arraylist
        size = items.size();
        //assign the size of the arraylist to an integer variable
        cart_size = Integer.toString(size);

        //Setting up values on edit texts of the food cart layout with values gained by intent
        if(size>0){
            item1.setText(items.get(0));

            //If quantity and size fields are empty set an error
            if (quantity1.length() == 0 && size1.length() == 0){
                quantity1.setError("Enter quantity");
                size1.setError("Enter size");
            }
            if(size>1){
                item2.setText(items.get(1));

                //If quantity and size fields are empty set an error
                if (quantity2.length() == 0 && size2.length() == 0){
                    quantity2.setError("Enter quantity");
                    size2.setError("Enter size");
                }
                if (size>2){
                    item3.setText(items.get(2));

                    //If quantity and size fields are empty set an error
                    if (quantity3.length() == 0 && size3.length() == 0){
                        quantity3.setError("Enter quantity");
                        size3.setError("Enter size");
                    }
                    if (size>3){
                        item4.setText(items.get(3));
                    }
                }
            }
        }

    }

    //Continue button method
    public void addToCart(View view){
        //Initializing dbHelperorder
        DBHelperOrder dbHelperOrder = new DBHelperOrder(this);

        //Initializing variables
        item_name1 = item_name2=item_name3=item_name4="item";
        item_quantity1=item_quantity2=item_quantity3=item_quantity4="1";
        item_size1=item_size2=item_size3=item_size4="small";

        //Retrieve the user entered values from the food cart layout
        if(size>0){
            item_name1 = item1.getText().toString();
            item_quantity1 = quantity1.getText().toString();
            item_size1 = size1.getText().toString();
            //Insert data to cart row table
            ID1 = dbHelperOrder.addCartRowTableInfo(item_name1,item_quantity1,item_size1);
            if(size>1){
                item_name2 = item2.getText().toString();
                item_quantity2 = quantity2.getText().toString();
                item_size2 = size2.getText().toString();
                //Insert data to cart row table
                ID2 = dbHelperOrder.addCartRowTableInfo(item_name2,item_quantity2,item_size2);
                if (size>2){
                    item_name3 = item3.getText().toString();
                    item_quantity3 = quantity3.getText().toString();
                    item_size3 = size3.getText().toString();
                    //Insert data to cart row table
                    ID3 = dbHelperOrder.addCartRowTableInfo(item_name3,item_quantity3,item_size3);
                    if (size>3){
                        item_name4 = item4.getText().toString();
                        item_quantity4 = quantity4.getText().toString();
                        item_size4 = size4.getText().toString();
                        //Insert data to cart row table
                        ID4 = dbHelperOrder.addCartRowTableInfo(item_name4,item_quantity4,item_size4);
                    }
                }
            }
        }

        //Navigate to Orderdetails.class
        Intent i1 = new Intent(FoodCart.this,OrderDetails.class);
        i1.putExtra("cart_size",cart_size);
        i1.putExtra("ID1",ID1);
        i1.putExtra("ID2",ID2);
        i1.putExtra("ID3",ID3);
        i1.putExtra("ID4",ID4);

        //Checking whether data got inserted successfully or not
        if (ID1!=null || ID2!=null || ID3!=null || ID4!=null){
            Toast.makeText(this, "Data successfully inserted ", Toast.LENGTH_SHORT).show();
        } else{
            Toast.makeText(this, "Data Not inserted!", Toast.LENGTH_SHORT).show();
        }

        startActivity(i1);
    }

    //Continue shopping button method
    public void continueShopping(View view){
        Intent i2 = new Intent(FoodCart.this,dashboard.class);
        startActivity(i2);
    }

}